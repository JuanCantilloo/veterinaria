...
def inicio():
    ...
    cur.execute("SELECT COUNT(*) AS total FROM mascotas")
    total_registradas = cur.fetchone()['total']

    cur.execute("SELECT COUNT(*) AS total FROM mascotas_adoptadas")
    total_adoptadas = cur.fetchone()['total']
...
def registrar_mascota():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO mascotas (nombre, tipo, raza, fecha_nacimiento, peso, color, descripcion, fecha_ingreso, foto)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (nombre, tipo, raza, fecha_nacimiento, peso, color, descripcion, fecha_ingreso, foto_path))
        conn.commit()
        cur.close()
        conn.close()
...
@app.route('/formulario_adopcion', methods=['GET', 'POST'])
def formulario_adopcion():
    ...
    if request.method == 'POST':
        ...
        cur.execute("SELECT * FROM mascotas_adoptadas WHERE mascota_id = %s", (mascota_id,))
        if cur.fetchone():
            flash('Esa mascota ya fue adoptada.')
            return redirect(url_for('formulario_adopcion'))

        # Registrar adopción
        cur.execute("INSERT INTO mascotas_adoptadas (mascota_id, fecha_adopcion) VALUES (%s, %s)", (mascota_id, date.today()))
        # Asignar propietario
        cur.execute("UPDATE mascotas SET usuario_id = %s WHERE id = %s", (usuario_id, mascota_id))

        conn.commit()
        flash('¡Adopción registrada exitosamente!')
    ...
    cur.execute("""
        ...
        FROM mascotas 
        WHERE id NOT IN (SELECT mascota_id FROM mascotas_adoptadas)
    """)
    mascotas = cur.fetchall()

    return render_template('adoptar_mascota.html', usuarios=usuarios, mascotas=mascotas)
...
def listar_mascotas():
    ...
    cur.execute("""
        ...
        FROM mascotas 
        LEFT JOIN usuarios ON mascotas.usuario_id = usuarios.id
        WHERE mascotas.id NOT IN (SELECT mascota_id FROM mascotas_adoptadas)
    """)
    mascotas = cur.fetchall()
...
def editar_mascota(id):
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
            ...
            color=%s,descripcion=%s, fecha_ingreso=%s, usuario_id=%s WHERE id=%s
        """, (data['nombre'], data['raza'], data['fecha_nacimiento'], data['peso'],
              data['color'],data['descripcion'], data['fecha_ingreso'], data['usuario_id'], id))
        conn.commit()
...
def adoptar_mascota(id):
    ...
    # Verificar si ya fue adoptada
    cur.execute("SELECT * FROM mascotas_adoptadas WHERE mascota_id = %s", (id,))
    if cur.fetchone():
        flash('La mascota ya fue adoptada.')
        return redirect(url_for('listar_mascotas'))
...
@app.route('/revertir_adopcion/<int:id>', methods=['POST'])
def revertir_adopcion(id):
    conn = connect_to_db()
    cur = conn.cursor()
    cur.execute("DELETE FROM mascotas_adoptadas WHERE mascota_id = %s", (id,))
    conn.commit()
    cur.close()
    conn.close()
    flash('Adopción revertida.')
    return redirect(url_for('listar_adoptadas'))
